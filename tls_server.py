...
class Session():
	def __init__(self, client_random, cipher_suite, protocol, server_random):
...
class ClientHello():
	def __init__(self, client_hello):
		if client_hello[:2] != HANDSHAKE_RECORD_TYPE
		self.protocol = 
...
class Server():
	...
	def run(self, socket):
		data = socket.receive()
		self.log_receiving(data.decode())
		self.hello()
